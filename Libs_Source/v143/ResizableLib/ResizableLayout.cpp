									   ...
									   CRgn *pRegion) const
{
	...
	const BOOL bClipping = layout.properties.bAskClipping ? LikesClipping(layout) : layout.properties.bCachedLikesClipping;
...
#if (_WIN32_WINNT >= 0x0501)
		if ((real_WIN32_WINNT >= 0x0501) && (real_ThemeSettings & STAP_ALLOW_NONCLIENT))
		{
			CWnd *pWnd = GetResizableWnd();
