...
int32_t CxImage::DrawString(HDC hdc, int32_t x, int32_t y, const char* text, RGBQUAD color, const char* font, int32_t lSize, int32_t lWeight, uint8_t bItalic, uint8_t bUnderline, bool bSetAlpha)
{
	if (IsValid()){
		...
		//wcscpy(m_pLF->lfFaceName, font) //wchar_t copy
...
////////////////////////////////////////////////////////////////////////////////
 int32_t CxImage::DrawString(HDC hdc, int32_t x, int32_t y, const wchar_t* text2, RGBQUAD color, const wchar_t* font2, int32_t lSize, int32_t lWeight, uint8_t bItalic, uint8_t bUnderline, bool bSetAlpha)
	 ////////////////////////////////////////////////////////////////////////////////
// For UNICODE support: char -> TCHAR
 {
	 if (IsValid()) {
		 char* text = UnicodeToAnsi(text2);
		 char* font= UnicodeToAnsi(font2);
		 //get the background
		 HDC pDC;
		 if (hdc) pDC = hdc; else pDC = ::GetDC(0);
		 if (pDC == NULL) return 0;
...
//char*转wchar_t*
wchar_t* CxImage::AnsiToUnicode(const char* szStr)
{
...
//wchar_t*ת转char*
char* CxImage::UnicodeToAnsi(const wchar_t* szStr)
{
